...
def generate_prompt_rules(task):
    if task == 'reasoning':
        ...
        "2. Please remember the meanings of the following identifiers: [Query] represents the event to be predicted in the form of (S, R, T). [Key Events] represents the list of relatively important historical events that have a significant impact on the forecast. [Relevant Events] represents the list of supplementary event that provides additional information and relevant context. [Options] represents the candidate set of the missing object.\n",
...
def run_llm(prompt,prompt_rules, temperature, max_tokens, gemini_api_keys, engine):
...
class LinkDataset(object):
    def __init__(self):
        with open('/mnt/dataDisk/hxli/MEE/Code_For_EF/gemini/doc_list.json', 'r') as file:
            self.doc = json.load(file)
        with open('/mnt/dataDisk/hxli/MEE/Code_For_EF/Data_gemini/sum_gemini_all.json', 'r') as file:
            self.doc_sum = json.load(file)
        with open('/mnt/dataDisk/hxli/MEE/Code_For_EF/gemini/Md52timid.json','r') as f:
            self.Md52timid = json.load(f)
        self.df_all = pd.read_csv("/mnt/dataDisk/hxli/MEE/Code_For_EF/gemini/all_dataset.csv",sep="\t",dtype={"Relation_id": str})

        with open('/mnt/dataDisk/hxli/MEE/Code_For_EF/Data_gemini/classification_gemini_all.json','r') as f:
            self.classification = json.load(f)
        with open('/mnt/dataDisk/hxli/MEE/Code_For_EF/Data_gemini/aligned_gemini_all.json','r') as f:
            self.aligned = json.load(f)
        with open('/mnt/dataDisk/hxli/MEE/Code_For_EF/Data_gemini/complementary_gemini_all_new.json','r') as f:
    ...
    def text_align(self, subject, text_list,args):
        ...
        for i in range(len(text_list)):
            ...
            prune_result = run_llm(prompt_summary, prompt_summary_rules, args.temperature, args.max_tokens, args.gemini_api_keys, args.engine)
            # TODO
    ...
    def retrieve_text(self, subject, ceid, timid, histlen, opt):
        ...
        '''for Md5 in Md5_list:
            ...
            sum_i = "* " + "* ".join(sum_i) 
            documents.append(Document(text=sum_i,metadata={'timid':self.Md52timid[Md5][0],'Md5':Md5,'title':self.doc[Md5]["Title"]}))

        print(documents)
        
        Settings.llm = llama_index_OpenAI(model="gpt-3.5-turbo", temperature=0.1, max_tokens=256)
        Settings.embed_model = OpenAIEmbedding(model="text-embedding-3-small", embed_batch_size=50)
...
def main(args):
    ...
    with tqdm(total=df.shape[0], desc="Event Forecasting") as pbar:
        for index_event, row in df.iterrows():
            ...
            # generate summary for each text(considering the length of the retrieved text)
            #complement_summary = database.text_align(row['Subject'],text_sort_by_timid,args)
            ...
            #print(prompt_reasoning_rules)
            result = run_llm(prompt_reasoning, prompt_reasoning_rules, args.temperature, args.max_tokens, args.gemini_api_keys, args.engine)
            sleep(2)
            ...
            if response == None:
                ...
                continue
            # TODO
            # print(f"prompt_reasoning_rules:\n{prompt_reasoning_rules}")
            # print(f"prompt_reasoning:\n{prompt_reasoning}")
            # print(f"result:\n{result}")
        ...
        with open("./result_retrieve_w_mm_new_compl_new.json", "w") as f: 
            json.dump(all_result, f, ensure_ascii=False, indent=2)
        print('save in ./result_retrieve_w_mm_new_compl_new.json')

        wrong_ev = {}
        wrong_ev['wrong_ev_index'] = wrong_event
        with open("./wrong_event_w_mm_new_compl_new.json", "w") as f: 
            json.dump(wrong_ev, f)

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--data_path", type=str, default="/mnt/dataDisk/hxli/MEE/Code_For_EF/gemini/final_test.csv")
    parser.add_argument("--gemini_api_keys", type=str, default="AIzaSyDjxVopAhoeQfnDb6dUwUgo_rVU7qI1dOY")
    parser.add_argument("--engine", type=str, default="gemini-1.0-pro")
    ...
    parser.add_argument("--related_number", type=int, default=30)
