...
class LinkDataset(object):
    ...
    def retrieve_text_contriever(self, relevant_events,subject, ceid, timid, histlen,contriever,tokenizer):
        ...
        if len(Md5_list)!=0:
            ...
            query_gt = tokenizer.batch_encode_plus([" ".join(relevant_events)], padding=True, truncation=True, return_tensors="pt")
            query_t = tokenizer.batch_encode_plus([subject], padding=True, truncation=True, return_tensors="pt")
            inputs = tokenizer.batch_encode_plus(documents, padding=True, truncation=True, return_tensors="pt")
            embeddings = contriever(**inputs)
            query_embeddings_gt = contriever(**query_gt)
            query_embeddings_t = contriever(**query_t)
            index = faiss.IndexFlatIP(768)
            index.add(embeddings.detach())
            k = 10
            # print(query_embeddings.shape)
            D_gt, I_gt = index.search(query_embeddings_gt.detach().reshape(1,-1), k)
            D_t, I_t = index.search(query_embeddings_t.detach().reshape(1,-1), k)
            response_gt = [[documents[index],doc_info[index]] for index in I_gt[0]]
            response_t = [[documents[index],doc_info[index]] for index in I_t[0]]
        else:
            response_gt = []
            response_t = []
...
def main(args):
    ...
    with tqdm(total=df.shape[0], desc="Event Forecasting") as pbar:
        for index_event, row in df.iterrows():
            ...
            retrieve_result_gt,retrieve_result_t = database.retrieve_text_contriever(relevant_subject_set,row['Subject'],row['ce_id'],row['timid'],histlen,contriever,tokenizer)
        ...
        with open(f"result_with_history_retrieve_graph_text_ce_contriever.json", "w") as f: 
            json.dump(all_result_gt, f, ensure_ascii=False, indent=2)
        with open(f"result_with_history_retrieve_text_ce_contriever.json", "w") as f: 
            json.dump(all_result_t, f, ensure_ascii=False, indent=2)

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    # add_model_args(parser)
    parser.add_argument("--data_path", type=str, default="/home/ysma/hchang_workspace/Event_Forecasting/data/all_events_final_test.csv")
