...
def main(args, generator, doc_list, md5_list, exists_results, start_idx, end_idx):
    ...
    for idx, md5 in tqdm(enumerate(md5_list), total=len(md5_list)):
        ...
        with open(args.input_path + 'raw_results/results/' + md5, 'r') as fraw:
            rsp = fraw.read()
            rsp = rsp.strip(' |\n' + punctuation)
            ...
            for event in events:
                ...
                if len(fields) != 3:
                    continue
                field = [ "[" + x + "]" for x in fields ]
                string = "; ".join(str(x) for x in field)
                msg = "You are an assistance to find a specific paragraph in an article based on an event, and output that paragraph.\n Each event in format: [event actor one]; [event relation]; [event actor two]. For example, given the example event: [Iran]; [Threaten]; [American naval forces].\n Now given the event " + string + "and reference article:  \n\n" + "\n".join(article["Text"]) + "\nNow, please give me the specific paragraph in this article."
                print(msg)
            try:
                ...
                with open(args.output_path + args.year + '/results/' + md5, 'a+') as fresult:
                    fresult.write(rsp)
...
if __name__ == "__main__":
    ...
    year = args.year
    mread = json.load(open(args.input_path + 'md5list.json'))
    md5_list = mread['md5']
    ...
    data_path = args.input_path + 'MIDEAST_doc.json'
