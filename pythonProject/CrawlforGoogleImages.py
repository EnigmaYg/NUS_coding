...
def get_image_url(num, key_word):
    ...
    while cnt != num:
        ...
        time.sleep(5)
        try:
            imgurl.append(driver.find_element_by_xpath('//*[@class="sFlh5c pT0Scc iPVvYb"]').get_attribute('src'))
            i += 1
            cnt += 1
            driver.back()
            time.sleep(4)
    ...
    for key, value in url.items():
        ...
        for i in value:
            ...
            try:
                ...
                print(key + i)
                pass
            # res = request.urlopen(url=i, timeout=3, headers=headers, context=ctx).read()
            key_ = get_md5(i)
            l.append(key_)
            path = "E:\\images\\" + key_
            write_in_in[key_] = i
...
if __name__ == '__main__':
    ...
    # with open('MD5document2photo.json', 'w') as f:
    #    json.dump(write_in_, f, indent=2)
    # with open('MD5photo2photo.json', 'w') as f:
